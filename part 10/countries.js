...
const allLanguage = countries.reduce((acc, country) => {
    return acc.concat(country.languages)
}, [])

const language = new Set(allLanguage);
...
const sortedLanguages = Object.entries(languageCount).sort((a, b) => b[1] - a[1]).map(entry => entry[0])
